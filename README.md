...
- `createKenBurnsVideoFromImages(options: IKenBurnsOptions): Promise<string>` - 从图片创建带有 Ken Burns 效果的视频，支持字幕和音频合成
  - `options.scenes`: 场景配置数组，每个场景包含图片、时长、字幕和音频等信息
  - `options.resolution`: 可选，输出视频分辨率，默认为 '1280x720'
  - `options.fadeDuration`: 可选，淡入淡出时长（秒），默认为 0.5
  - `options.fps`: 可选，帧率，默认为 30
...
```typescript
import { createKenBurnsVideoFromImages } from 'coze-plugin-utils';

async function generateKenBurnsVideo() {
  try {
    const videoPath = await createKenBurnsVideoFromImages({
      scenes: [
        {
    ...
    console.log('Ken Burns 视频生成完成:', videoPath);
  } catch (error) {
